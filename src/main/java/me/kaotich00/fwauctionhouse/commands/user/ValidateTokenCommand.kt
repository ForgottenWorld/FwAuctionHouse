...
class ValidateTokenCommand : UserCommand() {
    override fun onCommand(sender: CommandSender, args: Array<String>) {
        ...
        val optPendingToken: Optional<PendingToken> =
            SimpleMarketService.Companion.getInstance()!!.getPendingToken(sessionId.toInt())
        if (optPendingToken.isPresent) {
            val pendingSell = optPendingToken.get()
            Message.VALIDATED_TOKEN.send(sender)
            StorageFactory.getInstance().storageMethod.validateToken(sessionId.toInt())
            SimpleMarketService.Companion.getInstance()!!.removeFromPendingToken(pendingSell)
        }
    }
